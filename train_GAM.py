...
class GAModel(torch.nn.Module):
    ...
    def forward(self, encoder_hidden_states, latents, ref_latents, timesteps):
...
def main():
    ...
    dataset = FGGDataset(
        ...
        tokenizer,
        image_root_path='/path_to/GarmentBench',
    )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                latents = vae.encode(
                    batch["vae_cloth"].to(accelerator.device, dtype=weight_dtype)).latent_dist.sample()
                latents = latents * 0.18215

                ref_latents = vae.encode(
                    batch["vae_sketch"].to(accelerator.device, dtype=weight_dtype)).latent_dist.sample()
